...
function PropertyForm({ services, workspace, listing, onSaved, onCancel }: { services: StudioServices; workspace: Workspace; listing?: Listing; onSaved: (id: string) => void; onCancel?: () => void }) {
  const readOnly = workspace.memberships.find((m) => m.orgId === workspace.org.id)?.role === "marketing";
...
function PropertyWorkspace({ services, workspace, listing, onChanged }: Props & { listing: Listing }) {
  ...
  const canWrite = workspace.memberships.find((m) => m.orgId === workspace.org.id)?.role !== "marketing";
  const refresh = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      if (!current.signal.aborted && alive.current) { setState(activity); setMedia(previews); setError(""); }
    } catch (reason) { if (!current.signal.aborted && alive.current) setError(message(reason)); }
