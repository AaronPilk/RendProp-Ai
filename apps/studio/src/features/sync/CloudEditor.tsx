...
export default function CloudEditor(props: CloudEditorProps) {
  ...
  const [initialConversation,setInitialConversation]=useState<ConversationState>();
  const conversation=useRef<ConversationState|undefined>(undefined);
  ...
  const externalBlock=useRef(props.onSwitchBlockChange);externalBlock.current=props.onSwitchBlockChange;
  const observeEditorBlock = useCallback((reason: string | null) => {editorBlock.current = reason;externalBlock.current?.(reason);}, []);
  useEffect(()=>()=>externalBlock.current?.(null),[]);
  ...
  useEffect(() => {
    ...
    void sync.open().then(async doc => {
      ...
      if (doc) {
        ...
        draft = saved.draft; sources.current = saved.sources;conversation.current=saved.conversation;setInitialConversation(saved.conversation);
        try {
        ...
        if (saved) {draft = saved.draft; sources.current = saved.sources;conversation.current=saved.conversation;setInitialConversation(saved.conversation);}
      }
  ...
  async function requestPromptEnhancement(message:string,draft:EditDraft,history:ConversationState["messages"],signal:AbortSignal){
    if(!ready||sourceBusy||outputBusy||state==="conflict")throw new Error("Finish syncing this edit before improving its prompt.");
    return assistant.requestPromptEnhancement(message,draft,history,signal);
  }
  return <div className="cloud-editor">
    ...
    <section className="sync-toolbar panel creation-source-actions">
