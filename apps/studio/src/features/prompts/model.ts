...
export function compilePrompt(recipeId: string, input: PromptInputs): PromptCompilation {
  ...
  if (isPhoto && !Number.isInteger(input.sourceSeconds)) throw new Error("Use a whole-second clip duration from 4 to 30 seconds for photo generation.");
