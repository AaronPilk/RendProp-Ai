...
type Props={workspace:Workspace|null;listings:Listing[];selectedId?:string;busy:boolean;spatialAvailable:boolean;onSelect:(id:string)=>void;onFeature:(id:FeatureId,listingId?:string)=>void;onCreate:()=>void;onProperties:()=>void;onLeads:()=>void;onPlanner:()=>void;onConnect:()=>void;onLibrary:()=>void;};
export default function Dashboard(props:Props) {
  ...
  return <div className="app-dashboard">
    <section className="app-home-hero">
      <div><span className="app-brand-kicker">RENDPROP</span><h1>Your phone captures it.<br/>Your studio brings it to life.</h1><p>The same {words.plural}. The same tools. More room to create.</p></div>
      <div className="app-hero-action"><button onClick={()=>workspace?props.onFeature("reel"):props.onConnect()}><Icon name="film" size={20}/>Make a reel<Icon name="arrow" size={18}/></button><span>{workspace?"Start with photos and video from your phone":"Sign in with the Apple account you use in the app"}</span></div>
    </section>
