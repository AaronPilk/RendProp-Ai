...
export function createStudioServices(
  ...
  async function refresh(version: number, rejectedToken: string) {
    ...
    // A parallel request or auto-refresh may already have replaced this token.
    ...
    if (!refreshFlight) {
      ...
      refreshFlight = (async () => {
        ...
        if (data.session.user.id.toLowerCase() !== snapshot.identity?.userId)
          throw new StudioError(
            "stale-identity",
            "The account changed while refreshing your session.",
          );
        // The SDK normally sends TOKEN_REFRESHED first. Never overwrite a later event.
  ...
  return {
    ...
    async listListings(orgId: string, signal?: AbortSignal) {
      ...
      return decodeListings(rows, selected, allowed);
    },
    async listMedia(
      orgId: string,
      listingId: string,
