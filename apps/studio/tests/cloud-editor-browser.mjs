...
try {
  ...
  async function device(seed) {
    ...
    await tab.goto(`${origin}/tests/cloud-editor-fixture.html`); return tab;
  }
  page = await device();
  ...
  await expect.poll(async () => (await page.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.sources?.length).toBe(1);
  await expect.poll(async () => (await page.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.draft?.title).toBe("Office to phone edit");
  ...
  await second.getByRole("combobox", { name: "Save this edit with", exact: true }).selectOption("20000000-0000-4000-8000-000000000003");
  await expect.poll(async () => (await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.sources?.[0]?.listingId).toBe("20000000-0000-4000-8000-000000000003");
  assert.equal((await second.evaluate(() => window.cloudFixture.snapshot())).tickets, 2);
  receipt.checks.push("Changing property saves originals to that property instead of silently retaining another property’s source");
  ...
  await expect.poll(async () => (await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.sources?.length).toBe(3);
  await expect.poll(async () => (await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.draft?.narration?.offset).toBe(0.15);
  const advanced = await second.evaluate(() => window.cloudFixture.snapshot());
  const third = await device(advanced);
  ...
  await second.evaluate(() => window.cloudFixture.requestOtherPropertyMedia());
  await expect(second.getByRole("button", {name:"Add files and move this edit",exact:true})).toBeVisible();
  await expect(second.getByLabel("Save this edit with", {exact:true})).toHaveValue("20000000-0000-4000-8000-000000000003");
  assert.equal((await second.evaluate(() => window.cloudFixture.snapshot())).tickets,beforeImport.tickets);
  await second.getByRole("button", {name:"Cancel import",exact:true}).click();
  receipt.checks.push("Library import from another property requires review and cannot silently upload into the saved edit’s old property");
  ...
  assert.deepEqual((await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit.payload.draft.clips,beforeImport.documents.edit.payload.draft.clips);
  ...
  await expect.poll(async () => (await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.sources?.length).toBe(2);
  const applied=await second.evaluate(() => window.cloudFixture.snapshot());assert.equal(applied.tickets,beforeImport.tickets);
  assert.equal(applied.documents.edit.payload.draft.clips[1].motion,"pan_left");
  ...
  await expect.poll(async () => (await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.sources?.length).toBe(2);
  await expect.poll(async () => (await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.draft?.overlays?.length).toBe(1);
  const agentSaved = await second.evaluate(() => window.cloudFixture.snapshot()), fourth = await device(agentSaved);
  ...
  await expect.poll(async () => (await second.evaluate(() => window.cloudFixture.snapshot())).documents.edit?.payload.sources?.length).toBe(0);
  receipt.checks.push("Removing a clip prunes its cloud document source reference while retaining original property media");
  await second.evaluate(() => window.cloudFixture.remoteEdit());
  await expect(second.getByRole("button", { name: "Reload saved version", exact: true })).toBeVisible();
  await expect(second.getByLabel("Title overlay", { exact: true })).toHaveValue("20 Pine Avenue");
  receipt.checks.push("Other-device edit surfaces a reload conflict and preserves the current local edit");
