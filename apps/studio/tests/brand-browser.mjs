...
try{
 ...
 await expect(picker).toContainText('22 Pine Street');await expect(page.locator('.reel-picker-context')).toContainText('Your current edit belongs to 10 Oak Street');
 await expect(page.getByLabel('Save this edit with',{exact:true})).toHaveValue(first);await expect(page.getByLabel('Title overlay',{exact:true})).toHaveValue('Office reel');await picker.getByRole('button',{name:'Close media picker',exact:true}).click();
 await expect(page.getByLabel('Save this edit with',{exact:true})).toHaveValue(first);await expect(page.getByLabel('Title overlay',{exact:true})).toHaveValue('Office reel');
 check('Opening another home’s reel picker and cancelling leaves the prior edit, source files and property assignment intact');
