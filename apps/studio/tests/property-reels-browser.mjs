...
try{
  ...
  const openEditor=async tab=>{await tab.getByRole("button",{name:"Pro view",exact:true}).click();await expect(title(tab)).toBeVisible();};
  const savedTitle=async(tab,id,text)=>expect.poll(async()=>(await snapshot(tab)).documents[`edit:${id}`]?.payload.draft.title).toBe(text);
  page=await device();await openEditor(page);await expect(title(page)).toBeVisible();
  ...
  await page.getByLabel("Property reel",{exact:true}).selectOption(b);await openEditor(page);await expect(title(page)).toHaveValue("");await expect(page.getByRole("button",{name:/Select clip /})).toHaveCount(0);
  ...
  await page.getByLabel("Property reel",{exact:true}).selectOption(a);await openEditor(page);await expect(title(page)).toHaveValue("Oak property story");await expect(page.getByRole("button",{name:/Select clip 1:/})).not.toHaveAccessibleName(/original file missing/);
  ...
  await page.evaluate(()=>window.cloudFixture.requestReelEntry("open-b","20000000-0000-4000-8000-000000000003"));await openEditor(page);await expect(title(page)).toHaveValue("Pine property story");
  ...
  await page.getByRole("button",{name:"Try opening 10 Oak Street again",exact:true}).click();await openEditor(page);await expect(title(page)).toHaveValue("Oak property story");
  ...
  const seed=await snapshot(page);const second=await device(seed);await openEditor(second);await expect(title(second)).toHaveValue("Oak property story");await expect(second.getByRole("button",{name:/Select clip 1:/})).not.toHaveAccessibleName(/original file missing/);await second.close();await page.bringToFront();
  ...
  await recovered.getByRole("button",{name:"Use earlier browser backup",exact:true}).click();await openEditor(recovered);await expect(title(recovered)).toHaveValue("Unsaved browser correction");await savedTitle(recovered,a,"Unsaved browser correction");
  ...
  await recovered.evaluate(()=>window.cloudFixture.switchAccount());await openEditor(recovered);await expect(title(recovered)).toHaveValue("");await recovered.evaluate(()=>window.cloudFixture.releaseReads());await expect(title(recovered)).toHaveValue("");
