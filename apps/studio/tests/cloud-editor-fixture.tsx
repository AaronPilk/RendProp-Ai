...
import CloudEditor from "../src/features/sync/CloudEditor";
...
const services = {
  ...
  upload: async (url: string, blob: Blob) => { const id = url.split("/").at(-1), asset = cloud.assets.find(a => a.id === id)!; let bytes = ""; for (const n of new Uint8Array(await blob.arrayBuffer())) bytes += String.fromCharCode(n); asset.base64 = btoa(bytes); cloud.puts++; persist(); return { etag: "fixture-receipt" }; },
  listMedia: async (_org: string, id: string) => ({ orgId: org, listingId: id, photos: cloud.assets.filter(a => a.listing_id === id && a.uploaded && a.kind === "photo").map(a => ({ id: a.id, listingId: id, url: URL.createObjectURL(new Blob([Uint8Array.from(atob(a.base64!), c => c.charCodeAt(0))], { type: a.content_type })), expiresAt: "2099-09-14T12:00:00Z", caption: "Editor source", isStaged: false, sort: 0 })), videos: cloud.assets.filter(a => a.listing_id === id && a.uploaded && a.kind === "video").map(a => ({id:a.id,listingId:id,url:URL.createObjectURL(new Blob([Uint8Array.from(atob(a.base64!), c => c.charCodeAt(0))], {type:a.content_type})),expiresAt:"2099-09-14T12:00:00Z",durationSeconds:4})), nextOffset: null, unavailableCount: 0 }),
  api: async (path: string, options: { method?: string; orgId: string; body?: Record<string, unknown> }) => {
    ...
    if (path.startsWith("/functions/v1/studio/documents")) {
      ...
      if (body) {
        const previous = cloud.documents[key]; if ((previous?.revision ?? 0) !== body.expected_revision) throw new StudioError("conflict", "Fixture concurrent save", 409);
        cloud.documents[key] = { key, kind: key, listing_id: null, payload: JSON.parse(canonicalDocument(body.payload)), revision: (previous?.revision ?? 0) + 1, updated_at: new Date().toISOString() }; cloud.writes++; persist();
      }
      return { document: structuredClone(cloud.documents[key] ?? null) };
    }
...
Object.assign(window, { cloudFixture: {
  ...
  remoteEdit: () => { const doc = cloud.documents.edit; cloud.documents.edit = { ...doc, revision: doc.revision + 1, payload: { ...doc.payload, draft: { ...(doc.payload.draft as object), title: "Saved on another device" } } }; persist(); window.dispatchEvent(new Event("focus")); },
} });
function Fixture() {
  ...
  return <div style={{ padding: 24 }}><button onClick={() => setPlanner(!planner)}>Switch to {planner ? "editor" : "planner"}</button>{notice && <p role="status">{notice}</p>}<div hidden={planner}><CloudEditor services={services} workspace={workspace} listings={listings} listingId={listing} active={!planner} importRequest={importRequest} importPlan={importPlan} importAgentPlan={importAgentPlan} onChanged={() => {}} /></div><div hidden={!planner}><CloudPlanner services={services} workspace={workspace} onNotice={setNotice} /></div></div>;
}
