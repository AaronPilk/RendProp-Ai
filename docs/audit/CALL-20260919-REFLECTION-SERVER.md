...
The real Bria smoke attempted by the parent task was rejected HTTP403, exhausted
...
- `POST /ai-video/declutter`: complete public video `asset_id`, matching `listing_id`,
  ...
  and stable UUID `Idempotency-Key`. It returns202 with an owned durable job ID and
...
- `POST /ai-video/declutter/cancel`: `batch_id` or `request_id`. Either cancels its
  ...
  arriving before the first POST creates a durable tombstone. Late submission409
  consumes nothing. An applied batch409 cannot be discarded retroactively.
...
Migration0055 defines service-only RPCs and RLS-closed job/batch tables. Every
...
`APP_AI_UNIT_CENTS` and `HANDOFF-DB.md` now use14¢/input second; the old4¢/clip
...
and refuses above240¢. The shared monthly-spend RPC includes unresolved cost
...
provider COGS. Definitive prequeue HTTP4xx rejection releases the unbilled hold;
5xx, timeout or lost response retains it for reconciliation. Stale dispatch claims
expire after2minutes and processing jobs after30minutes when status, quote or a
...
movie,≤2MiB metadata,≤64 box reads,15s overall read deadline. It skips media payload
...
Audio/video priming differences are bounded to150ms. Invalid or inconsistent
...
- Clean native PostgreSQL17: every migration0001–0055 in order;0055 replayed;
  transactional authorization/quota/cost/refund/provenance/deletion assertions
...
monthly-ceiling races. PostgreSQL16 container CI now has
...
- Four real ffmpeg source files (fast-start/tail metadata, audio,4.8s/10s/600s)
  pass the actual probe. A real10s file with only its movie duration forged to3s
  ...
  The600.75s fixture parses correctly but is refused by the≤600s apply boundary.
- Tour-host TypeScript check and all seven npm gates:2698 assertions pass.
  A separate actual-renderer video disclosure check passes25 assertions for
