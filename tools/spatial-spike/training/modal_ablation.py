...
def prepare(dataset, label):
    room.require(re.fullmatch(r"a[0-9]{2}", label) is not None, "only reviewed pose-only A attempts are configured")
    ...
    room.require(files == original["dataset_files"], "A requires the exact original 153-frame dataset")
    ...
    for path in sorted(PRIVATE_ROOT.glob(MARKER_GLOB)):
        ...
        room.require(receipt.get("phase") == "terminated" and
                     receipt.get("remote_copy_delete", {}).get("response") == "success" and
    ...
    return {"schema_version": 1, "label": label, "app_name": f"rendprop-spatial-ablation-{label}-20260914",
            ...
            "automatic_retries": 0, "pose_optimization": True, "steps": 3000,
            "max_training_seconds": 900, "max_gaussians": 500000,
...
def main():
    ...
    if args.command == "plan":
        ...
        print("PASS: exact A plan saved; no provider allocation")
        return
