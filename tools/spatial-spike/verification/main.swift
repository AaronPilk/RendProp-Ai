...
do {
    ...
    try require(args.count == 2 || (args.count == 3 && args[2] == "--transpose-first-pose"),
        "Usage: synthetic-capture /new/output/directory [--transpose-first-pose]")
    ...
    try Data("SYNTHETIC NOT A ROOM. Generated by a Swift CLI to test capture serialization and Python preparation. No phone capture, reconstruction, GPU training, or real scene.\n".utf8)
        .write(to: output.appendingPathComponent("SYNTHETIC-NOT-A-ROOM.txt"), options: .withoutOverwriting)
    ...
    manifest.status_detail = negative ? "SYNTHETIC invalid transposed-pose fixture; MUST be refused." : "SYNTHETIC serialization fixture; NOT phone capture or reconstruction."
    ...
    print("Wrote SYNTHETIC NOT A ROOM: 20 native JPEGs, 20 Swift sidecars, 120 unique points; \(negative ? "invalid transpose expected" : "Swift validation passed")")
} catch {
    fputs("FAIL: \(error)\n", stderr)
    exit(1)
}
