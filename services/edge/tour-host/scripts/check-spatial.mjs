...
export const sogMeta = {version:2,count:2048,means:{files:['means_l.webp','means_u.webp']},quats:{files:['quats.webp']},scales:{files:['scales.webp']},sh0:{files:['sh0.webp']}};
export function webp(width=64,height=32) {
...
export function sogFiles(meta=sogMeta) {return [['meta.json',Buffer.from(JSON.stringify(meta))],...['means_l.webp','means_u.webp','quats.webp','scales.webp','sh0.webp'].map(name=>[name,webp()])];}
...
// check-bundle.mjs imports the fixtures above; the checks only run for `node scripts/check-spatial.mjs`.
